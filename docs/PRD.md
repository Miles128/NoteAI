...
- `rag_chat` / `rag_chat_with_actions`（后者已等同于 `rag_chat`，无代码执行）。
- `rag_clear_memory` 清空 L2 记忆。

### 3.8 CLI Agent 桥接

支持从 NoteAI AI 面板直接调用第三方 CLI agent，以当前工作区为上下文操作 vault。

| 能力 | 说明 |
|------|------|
| 列出 agent | `list_cli_agents` 返回 claude / opencode / codex / gemini 的安装状态 |
| 派发任务 | `run_cli_agent` 以 workspace 为 cwd 启动子进程，stdout 流式回传 |
| Vault 指南 | `generate_vault_agents_md` 生成 `<workspace>/AGENTS.md`，描述三层架构、主题体系、笔记规范 |
| 事件类型 | `cli_agent_start` / `cli_agent_output` / `cli_agent_done` / `cli_agent_error` |

实现文件：`python/sidecar/cli_agent_runner.py`、`python/sidecar/handlers/cli_agent_handler.py`、`python/sidecar/vault_agents_md.py`。
