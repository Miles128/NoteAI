...
统计信息：
- Abstract 文件夹中的文件数量：{organized_count}
...
TOPIC_COUNT_SPECIFIED_INSTRUCTIONS_PROMPT = """主题数量要求（必须严格遵守）：
- 必须恰好返回 {topic_count} 个主题
- 不要多返回，也不要少返回"""


OUTPUT_FORMAT_SPECIFIED_INSTRUCTIONS_PROMPT = """输出格式：每行一个主题名称，共 {topic_count} 个，不要编号，不要描述，不要其他任何内容。例如：
机器学习
数据结构
网络协议"""


TOPIC_COUNT_AUTO_INSTRUCTIONS_PROMPT = """主题数量要求（请根据内容选择最优解）：
- 最少 {min_topics} 个主题
- 最多 {max_topics} 个主题
...
OUTPUT_FORMAT_AUTO_INSTRUCTIONS_PROMPT = """输出格式：每行一个主题名称，不要编号，不要描述，不要其他任何内容。例如：
机器学习
数据结构
网络协议"""
