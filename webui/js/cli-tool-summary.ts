...
(function() {
    ...
    function t(key: any, fallback: any, vars?: any) {
        if (window.t) {
            try { return window.t(key, vars || {}); } catch (_e) { /* ignore */ }
        }
        var s = fallback;
        if (vars) {
            Object.keys(vars).forEach(function(k: any) {
    ...
    function clip(text: any, max: any) {
    ...
    function normalizeToolName(name: any) {
    ...
    var VAULT_RESULT: any = {
        ...
        vault_search_notes: function(input: any, result: any) {
        ...
        vault_list_notes: function(_input: any, result: any) {
            var m = String(result || '').match(/Found (\d+)/i);
            if (m) {
    ...
    function describeResult(toolName: any, input: any, result: any, success: any) {
        var name = normalizeToolName(toolName);
        var args = parseMaybeJson(input) || input || {};
        var text = '';
        if (typeof result === 'string') {
            ...
            if (result.content && Array.isArray(result.content)) {
                text = result.content.map(function(c: any) { return c && c.text; }).filter(Boolean).join('\n');
            } else if (result.text) {
                text = result.text;
    ...
    function describeRunning(toolName: any, input: any) {
